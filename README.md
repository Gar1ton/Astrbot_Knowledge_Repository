...
[![version](https://img.shields.io/badge/版本-v0.29.0-blueviolet)](metadata.yaml)
...
| `/ka zotero account replace` | 确认换号：清空本地 Zotero 镜像并拉取新账号 |
| `/ka zotero account cancel` | 取消换号，保留旧 token 和全部本地数据 |
| `/ka r2 push` | 增量上传备份到 Cloudflare R2 |
| `/ka r2 force push` | 全量覆盖上传（需 60s 内重发确认） |
| `/ka r2 pull` | 下载并校验最新整库快照，登记恢复并等待手动重启（需确认） |
| `/ka r2 force pull` | 下载、校验、整体恢复并自动软重启（需确认） |
| `/ka r2 status` | 查看当前任务、Bucket 总占用、插件占用和最新快照 |
...
v0.29.0 使用独立的 `knowledge-arch/v1/` 协议。每次 `push` 都生成完整文件清单，但只上传远端不存在的 SHA-256 blob；`force push` 会重新上传全部 blob。`latest.json` 最后提交，因此上传或校验失败不会破坏上一份可恢复快照。提交成功后清理旧 manifest 和无引用 blob，远端只保留最新完整快照。

备份包含：

- `knowledge_repository.db` 的一致性 SQLite 快照，其中包括 collection、文档、chunks、笔记、聊天、同步和构建状态；
- `library/` 下原件、`clean.md`、`pages.json`、`meta.json`，以及复制进快照的外部 linked Zotero 原件；
- Milvus Lite 数据库、LightRAG workspaces、`embedding_cache.db`、`index_compatibility.json`；
- `runtime_config.json` 中可移植的非机密模型、索引和功能配置。

备份明确排除：R2/Zotero/LLM API key、Web 密码、`secrets/`、依赖包、模型缓存、日志、uploads、Zotero 下载缓存、临时任务文件、端口和设备绝对路径。换设备后需要先安装项目依赖，并在目标设备配置能访问同一 Bucket 的 R2 凭据。

跨设备恢复步骤：

1. 在新设备安装插件及所需依赖，配置原 R2 Bucket 的账号、密钥和 Bucket 名称。
2. 在设置页点击“一键恢复”，或执行 `/ka r2 force pull` 并二次确认。
3. 插件下载全部 blob，校验 manifest、SHA-256、路径安全和 SQLite integrity，再自动软重启。
4. 恢复整体覆盖目标设备的插件数据库、原件和索引；初始化失败会使用本地临时 rollback 恢复旧环境。密钥和设备路径始终保留目标设备现有值。

设置页与 `/ka r2 status` 同时展示整个 Bucket 物理占用和 `knowledge-arch/v1/` 插件物理占用；设置页另显示当前快照逻辑大小、去重节省量、对象数和更新时间。Bucket 用量读取失败时会阻断上传，避免把未知用量当作 0 放行。

> 恢复不做增量合并，会整体覆盖本地持久化数据。执行前确认远端 latest 快照就是需要的版本。

### Zotero Web API 换号保护

同一 Zotero 账号更换 token 会直接更新凭据，不清理数据。若新 token 属于不同账号，WebUI 会先显示与现有设计一致的确认弹窗，只提供“覆盖并重置本地 Zotero 库”和“取消更改”：

- 确认后仅删除 Zotero-origin 的本地镜像、集合、文档、chunks、制品、Milvus/LightRAG 贡献和相关状态；LOCAL 上传与本地 collection 保留，然后自动启动新账号完整 pull。
- 取消后旧 token 和全部本地数据保持不变。
- 环境变量或外部配置导致 `/ka zotero pull` 检测到换号时，pull 会被阻止；使用 `/ka zotero account replace` 或 `/ka zotero account cancel` 处理。
