...
| `web/server.py` | HTTP 层：请求参数翻译、鉴权、路由注册 | 委派给 `kacore/api.py` |
| `kacore/api.py` | 框架无关业务门面 | 组合 managers / pipelines / repository |
| `kacore/managers` / `kacore/pipelines` | 业务编排和多步骤流水线 | 依赖 repository 接口与 domain |
| `kacore/repository` | 持久化抽象与生产/内存实现 | 只依赖 domain |
| `kacore/domain` | 纯数据模型 | 零依赖 |
| `kacore/adapters` | 外部系统数据翻译 | 可依赖外部 SDK 与 domain |
