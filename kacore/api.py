...
from kacore.api_memecho import MemEchoApiMixin
...
class KnowledgeRepositoryApi(CapabilitiesApiMixin, RuntimeModelsApiMixin, MemEchoApiMixin):
    ...
    def __init__(
        ...
        *,
        ...
        # MemEcho 召回编排（组合根按 memecho.enabled 在 api 构造后注入；未启用为 None）。
        self._memecho_recall: Any | None = None
    ...
    async def ask(
        ...
        if retrieval_mode not in VALID_RETRIEVAL_MODES:
            raise ValueError(
                ...
                "'graph_only', 'deep_thinking', 'fulltext', or 'memecho'"
            )
        ...
        async def _retrieve_and_generate() -> dict:
            ...
            # memecho: 二选一独立召回——跳过本地向量/词法/图谱，直接查 MemoryEcho 记忆库并合成作答。
            if retrieval_mode == MODE_MEMECHO:
                return await self._ask_memecho(
                    question=question,
                    cid=cid,
                    answer_language=answer_language,
                    ask_start=ask_start,
