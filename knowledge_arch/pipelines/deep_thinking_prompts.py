...
from knowledge_arch.domain.deep_thinking import ChecklistItem
from knowledge_arch.pipelines.answer_synthesis import source_tag

if TYPE_CHECKING:
    from knowledge_arch.domain.deep_thinking import Checklist, EvidenceItem
...
def build_plan_prompt(query: str, max_sub_queries: int) -> str:
    return (
        ...
        "若问题涉及「这批文献」「多篇论文」「共同探讨」「横向对比」「纵观文献」，"
        "sub_queries 必须覆盖候选论文名/年份/arXiv id、数据集名、模型名、"
        "方法名、表格/图/章节锚点，避免只查一个泛化概念；"
        "不要生成答案，只规划需要哪些证据。"
    )
