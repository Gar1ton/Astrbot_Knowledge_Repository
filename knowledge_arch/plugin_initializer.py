...
from knowledge_arch.capabilities import module_available as _module_available
from knowledge_arch.config import Config, merge_config_dicts
from knowledge_arch.domain.models import Collection, SyncTargetKind
...
def _load_plugin_web_build_app() -> Any:
    """Load this plugin's web server without using the global ``ka_web`` package name."""
    server_path = Path(__file__).resolve().parent.parent / "ka_web" / "server.py"
...
class PluginInitializer:
    ...
    async def initialize(self) -> None:
        ...
        from knowledge_arch.managers.r2_backup_manager import apply_pending_restore
        ...
        from knowledge_arch.index_compatibility import (
        ...
        if (vdb_cfg.backend == "milvus" or graph_cfg.enabled) and embedding_runtime_available:
            from knowledge_arch.repository.embedding.factory import EmbeddingProviderFactory
        ...
        if (
            ...
            from knowledge_arch.repository.vector_store.milvus_lite import (
        ...
        if (
            ...
            if not _module_available("lightrag"):
                ...
                from knowledge_arch.lightrag_core import LightRAGCoreRegistry
                ...
                if graph_cfg.lightrag_llm_provider in {"local", "api"}:
                    if graph_cfg.lightrag_llm_base_url and graph_cfg.lightrag_llm_model:
                        ...
                        from knowledge_arch.adapters.llm import LMStudioLLMAdapter
                        from knowledge_arch.config import ENV_LIGHTRAG_LLM_API_KEY
        ...
        from knowledge_arch.repository.reranker import build_reranker
        ...
        if dt_cfg.llm_base_url and dt_cfg.llm_model:
            from knowledge_arch.adapters.llm import LMStudioLLMAdapter
            from knowledge_arch.config import ENV_DEEP_THINKING_LLM_API_KEY
        ...
        from knowledge_arch.managers.r2_backup_manager import R2BackupManager
        ...
        from knowledge_arch.research_skill import ResearchService
        ...
        from knowledge_arch.managers.r2_backup_manager import commit_applied_restore
    ...
    async def reload(self) -> None:
        ...
        try:
            ...
            from knowledge_arch.managers.r2_backup_manager import rollback_applied_restore
